...
public class SendSocketService {
    ...
    public static void sendMessageByFile(String filePath) {
        ...
        try {
            ...
            //说明不存在该文件
            if (!file.exists()) {
                EventBus.getDefault().post(new MessageBean(BltConstant.SEND_FILE_NOTEXIT));
                return;
            }

            //说明该文件是一个文件夹
            if (file.isDirectory()) {
                EventBus.getDefault().post(new MessageBean(BltConstant.SEND_FILE_IS_FOLDER));
                return;
            }
            ...
            //outputStream.write("\n".getBytes());
            outputStream.flush();
            EventBus.getDefault().post(new MessageBean(BltConstant.SEND_FILE_SUCCESS));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
